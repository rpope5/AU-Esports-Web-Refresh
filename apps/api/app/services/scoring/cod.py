...
def _build_inputs(payload: Any) -> tuple[CODInputs, float, float | None, dict[str, Any]]:
    current_rank_numeric = cod_rank_to_numeric(payload.profile.current_rank_label)
    peak_rank_numeric = (
        cod_rank_to_numeric(payload.profile.peak_rank_label)
...
def score_cod(payload: Any) -> ScoringResult:
