...
ROCKET_LEAGUE_ROLE_OPTIONS = {"Striker", "Third Man", "Support", "Flex"}

def rocket_league_rank_to_numeric(rank_label: str) -> float:
    if not rank_label:
        return 0.0
    ...
    rank_map = {
        ...
        "Champion": 82.0,
        "Grand Champion": 94.0,
        "Supersonic Legend": 100.0,
    }

    return rank_map.get(rank_label.strip().title(), 0.0)

@dataclass
class RocketLeagueInputs:
    rank_numeric: float
    hours_per_week: int
...
def _build_inputs(payload: Any) -> tuple[RocketLeagueInputs, float, float | None, dict[str, Any]]:
    current_rank_numeric = rocket_league_rank_to_numeric(payload.profile.current_rank_label)
    peak_rank_numeric = (
        rocket_league_rank_to_numeric(payload.profile.peak_rank_label)
...
def score_rocket_league(payload: Any) -> ScoringResult:
